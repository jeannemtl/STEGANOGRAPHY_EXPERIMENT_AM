...
class TrueAMSteganographyGenerator:
    ...
    def generate_am_signal(self, message: str, num_steps: int = 40) -> Tuple[List[float], List[int]]:
        """
        ...
        for step in range(num_steps):
            ...
            # Message bit (slow modulation envelope)
            bit_idx = int((step * len(binary_data)) / num_steps) % len(binary_data)
            message_bit = binary_data[bit_idx]
            
            # Modulation: bit 1 = high amplitude, bit 0 = low amplitude
            modulation = 1.0 if message_bit == 1 else (1.0 - self.modulation_depth)
