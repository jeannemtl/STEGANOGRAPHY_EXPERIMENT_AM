...
def analyze_step_by_step_representations(detector, dataset, max_examples_per_category=10):
    ...
    color_map = {
        ...
        'am_hello': 'cyan',              # Old template-based
        'am_secret': 'magenta',           # Old template-based
        'am_ai_risk': 'yellow',           # Old template-based
        'true_am_hello': 'darkturquoise',     # New continuous AM
        'true_am_secret': 'darkviolet',       # New continuous AM
        'true_am_ai_risk': 'gold'             # New continuous AM
    }
...
def create_animated_step_evolution(step_data, output_filename='step_evolution_animation.gif'):
    ...
    color_map = {
        ...
        'am_secret': 'magenta',
        'am_ai_risk': 'yellow',
        'true_am_hello': 'darkturquoise',     
        'true_am_secret': 'darkviolet',       
        'true_am_ai_risk': 'gold'
    }
    ...
    def animate(frame_step):
        ...
        # Enhanced legend including ALL AM patterns
        legend_elements = [
            ...
            Line2D([0], [0], marker='o', color='w', markerfacecolor='cyan', markersize=8, label='AM Hello (Template)'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='magenta', markersize=8, label='AM Secret (Template)'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='yellow', markersize=8, label='AM AI_Risk (Template)'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='darkturquoise', markersize=8, label='True AM Hello'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='darkviolet', markersize=8, label='True AM Secret'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='gold', markersize=8, label='True AM AI_Risk')
        ]
...
def main():
    ...
    # Load TRUE AM patterns with continuous modulation
    try:
        with open('true_am_steganographic_data.json', 'r') as f:
            true_am_data = json.load(f)
        
        for result in true_am_data['results']:
            message = result['message']
            reasoning_chain = result['reasoning_steps']  # It's already a list
            dataset[f'true_am_{message.lower()}'] = [reasoning_chain]
        
        print(f"Added {len(true_am_data['results'])} TRUE AM patterns with continuous modulation")
    except FileNotFoundError:
        print("True AM data not found")
